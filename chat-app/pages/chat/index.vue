...
<template>
  <div
        ...
        <div class="flex h-16 justify-between items-center">
          <div class="flex items-center space-x-2">
            <div class="flex-shrink-0">
              <div class="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-3 py-1.5 rounded-lg">
                <h1 class="text-xl font-bold">Work Chat</h1>
              </div>
            </div>
          </div>
          <div class="flex items-center space-x-4">
            ...
            <!-- ในส่วน header -->
            <div class="w-8 h-8 rounded-full flex items-center justify-center cursor-pointer" :class="currentUser ?
              ...
              <!-- สำหรับกรณีไม่มีไอคอน หรือข้อมูลผู้ใช้ -->
              ...
              <!-- สำหรับกรณีมีไอคอน -->
              <template v-else>
                <span class="text-white">{{ getUserAvatarIcon(currentUser as User) }}</span>
              </template>
            </div>

            <!-- <div class="flex items-center space-x-2">
              <div class="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-800 flex items-center justify-center">
            ...
            <button @click="logout"
              class="rounded-full bg-white dark:bg-gray-700 px-4 py-2 text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 border border-red-200 dark:border-red-800/30 transition-all duration-200 hover:shadow-md">
    ...
    <!-- Chat Interface -->
    <div class="container mx-auto py-6 px-4 flex gap-6 h-[calc(100vh-4rem)]">
      <!-- User List -->
      <div
        class="w-1/3 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl shadow-lg overflow-hidden flex flex-col transition-all duration-300 border border-gray-100 dark:border-gray-700/50">
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          <div v-else>
            ...
            <div v-for="user in users" :key="user.id" @click="selectUser(user)"
              class="p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer transition-all duration-200"
              :class="{ 'bg-blue-50/80 dark:bg-blue-900/20 border-l-4 border-blue-500 dark:border-blue-400': selectedUser?.id === user.id }">
              <div class="flex items-center space-x-3">
                <div class="relative">
                  ...
                  <div class="w-12 h-12 rounded-full flex items-center justify-center shadow-md"
                    ...
                    <span v-else class="text-white text-lg">{{ getUserAvatarIcon(user) }}</span>
                  </div>
                  <!-- Online Status -->
      ...
      <!-- Chat Area -->
      <div
        class="w-2/3 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl shadow-lg flex flex-col transition-all duration-300 border border-gray-100 dark:border-gray-700/50">
        <div v-if="!selectedUser"
          class="flex-1 flex flex-col items-center justify-center text-gray-500 dark:text-gray-400 p-6">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-20 w-20 text-gray-300 dark:text-gray-600 mb-4" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1"
              d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
          </svg>
          <p class="text-lg font-medium mb-2">Select a user to start chatting</p>
          <p class="text-sm text-center max-w-md">Choose a contact from the left panel to start a conversation or
            continue where you left off.</p>
        </div>
        <template v-else>
          <!-- Chat Header -->
          <div
            class="p-4 border-b dark:border-gray-700/50 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-t-xl">
            <div class="flex items-center space-x-3">
              <div class="relative">
                <div
                  class="w-12 h-12 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 dark:from-blue-500 dark:to-indigo-600 flex items-center justify-center shadow-md">
              ...
              <div>
                <h3 class="font-medium text-gray-900 dark:text-white text-lg">{{ selectedUser.name }}</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 flex items-center">
              ...
              <div v-if="userIsTyping" class="text-xs text-gray-500 dark:text-gray-400 mb-2 flex items-center">
                <div class="flex space-x-1 mr-2">
                  <div class="w-2 h-2 rounded-full bg-gray-400 dark:bg-gray-500 animate-bounce"
          ...
          <!-- Messages -->
          <div class="flex-1 overflow-y-auto py-6 px-4 space-y-4 chat-background transition-colors duration-200"
            ...
            <div v-for="message in messages" :key="message.id" class="flex"
              ...
              <div class="max-w-[70%]" :class="[
                ...
                <div class="rounded-2xl px-4 py-3 shadow-sm" :class="[
                  message.sender_id === currentUser?.id
                  ...
                  <p v-if="message.message_type === 'text'" v-html="formatEmojis(message.content)"></p>

                  <div v-else-if="message.message_type === 'image'" class="my-1">
                    <img :src="getImageUrl(message.file_url)" :alt="message.content"
                      class="max-w-full rounded-lg max-h-64 cursor-pointer hover:opacity-90 transition-opacity duration-200"
                      @click="message.file_url && openImagePreview(getImageUrl(message.file_url))" />
                    <p v-if="message.content" class="mt-2 text-sm" v-html="formatEmojis(message.content)"></p>
                  </div>

                  <span class="text-xs mt-1 block"
                    :class="message.sender_id === currentUser?.id ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'">
              ...
              <div v-if="message.sender_id !== currentUser?.id"
                class="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center mr-2 self-end order-1"
                :class="getSenderAvatarClass(message.sender_id)">
                <span v-if="!getSenderAvatarIcon(message.sender_id)" class="text-white font-medium text-xs">
                  {{ getSenderInitial(message.sender_id) }}
                </span>
                <span v-else class="text-white text-xs">{{ getSenderAvatarIcon(message.sender_id) }}</span>
              </div>
            </div>
          </div>
          ...
          <!-- Message Input -->
          <div
            class="p-4 border-t dark:border-gray-700/50 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-b-xl">

            <form @submit.prevent="sendMessage" class="flex space-x-2">
              <label
                class="p-2.5 bg-gray-50 dark:bg-gray-700 rounded-full text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer transition-colors duration-200">
              ...
              <div class="flex-1 relative">
                <input v-model="newMessage" type="text" placeholder="Type your message..."
                  class="w-full pl-4 pr-12 py-2.5 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-gray-700 dark:text-gray-200 transition-colors duration-200"
                  @input="handleTyping" />
                <div
                  class="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center space-x-1 text-gray-400">
                  <!-- Add this next to your file upload button in the form -->
                  <button type="button" @click="toggleEmojiPicker"
                    class="p-2.5 bg-gray-50 dark:bg-gray-700 rounded-full text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                      stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <button type="submit"
                class="rounded-full px-4 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-medium shadow-md hover:shadow-lg transition-all duration-200 flex items-center"
                :disabled="!newMessage.trim() && !selectedFile"
                :class="{ 'opacity-50 cursor-not-allowed': !newMessage.trim() && !selectedFile }">
            ...
            <div v-if="selectedFile"
              class="mt-2 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg flex items-center justify-between transition-colors duration-200 border border-gray-200 dark:border-gray-600">
              <div class="flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-blue-500 dark:text-blue-400 mr-2"
                  fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <span class="text-sm truncate dark:text-white">{{ selectedFile.name }}</span>
              </div>
              <button @click="clearSelectedFile"
                class="p-1.5 bg-red-50 dark:bg-red-900/20 rounded-full text-red-500 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors duration-200">
    ...
    <div v-if="showEmojiPicker"
      ...
      <div class="flex border-b border-gray-200 dark:border-gray-700 mb-2">
        <button v-for="(category, index) in ['😀', '🎁', '❤️', '🏠', '🚗']" :key="index" @click="activeCategory = index"
          class="px-2 py-1 text-center text-sm transition-colors duration-150"
          :class="activeCategory === index ? 'border-b-2 border-blue-500 dark:border-blue-400' : 'text-gray-500 dark:text-gray-400'">
          {{ category }}
        </button>
      </div>
    ...
    <div v-if="showAvatarModal"
      ...
      <div
        class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full mx-4 overflow-hidden border border-gray-200 dark:border-gray-700 transform transition-all duration-300">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
          <h3 class="text-lg font-medium text-gray-900 dark:text-white">Choose Profile Avatar</h3>
        ...
        <div class="px-6 py-4">
          <div class="mb-4">
            <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Background Color</h4>
            <div class="flex flex-wrap gap-2">
              <button v-for="color in backgroundColors" :key="color.value" @click="selectBackgroundColor(color.value)"
                class="w-8 h-8 rounded-full border-2 transition-all duration-200" :class="[
                  color.value === selectedBgColor ? 'border-blue-500 scale-110' : 'border-transparent',
                  color.class
                ]"></button>
            </div>
          </div>
          ...
          <div class="mb-4">
            <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Icon</h4>
            <div class="grid grid-cols-6 gap-3">
              <button v-for="(icon, index) in avatarIcons" :key="index" @click="selectIcon(index)"
                ...
                :class="[
                  selectedIconIndex === index ? 'border-blue-500 scale-110' : 'border-transparent',
                  selectedBgColor
                ]">
                <span v-if="!icon" class="text-white font-bold text-xl">
                  {{ currentUser?.name?.charAt(0).toUpperCase() }}
                </span>
                <span v-else class="text-white text-xl">{{ icon }}</span>
              </button>
            </div>
          </div>
          ...
          <div class="mb-6">
            ...
            <div class="flex items-center justify-center">
              <div class="w-20 h-20 rounded-full flex items-center justify-center shadow-lg" :class="selectedBgColor">
                <span v-if="!avatarIcons[selectedIconIndex]" class="text-white font-bold text-3xl">
                  {{ currentUser?.name?.charAt(0).toUpperCase() }}
                </span>
                <span v-else class="text-white text-3xl">{{ avatarIcons[selectedIconIndex] }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-3">
          ...
          <button @click="saveAvatar"
            class="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium transition-colors duration-200">
            Save
          </button>
        </div>
      </div>
...
const backgroundColors = [
  { value: 'bg-gradient-to-br from-blue-400 to-indigo-500', class: 'bg-gradient-to-br from-blue-400 to-indigo-500' },
  { value: 'bg-gradient-to-br from-purple-400 to-pink-500', class: 'bg-gradient-to-br from-purple-400 to-pink-500' },
  { value: 'bg-gradient-to-br from-red-400 to-orange-500', class: 'bg-gradient-to-br from-red-400 to-orange-500' },
  { value: 'bg-gradient-to-br from-green-400 to-teal-500', class: 'bg-gradient-to-br from-green-400 to-teal-500' },
  { value: 'bg-gradient-to-br from-yellow-400 to-orange-500', class: 'bg-gradient-to-br from-yellow-400 to-orange-500' },
  { value: 'bg-gradient-to-br from-pink-400 to-purple-500', class: 'bg-gradient-to-br from-pink-400 to-purple-500' },
  { value: 'bg-gradient-to-br from-teal-400 to-blue-500', class: 'bg-gradient-to-br from-teal-400 to-blue-500' },
  { value: 'bg-gradient-to-br from-indigo-400 to-purple-500', class: 'bg-gradient-to-br from-indigo-400 to-purple-500' },
];
...
const selectUser = async (user: User) => {
  ...
  try {
    ...
    messages.value = (data.value || []).sort((a: Message, b: Message) =>
...
const resizeImage = async (file: File, maxWidth = 1024, maxHeight = 1024, quality = 0.8): Promise<File> => {
